...
pub async fn create(
    ...
    if count == 0 {
        ...
        sqlx::query!(
            "INSERT INTO event_modifiers (id, webhook_id, events, repo_id, blacklisted, whitelisted, redirect_channel, guild_id, priority) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)",
            modifier_id,
            webhook_id,
            &events,
            parsed_repo_id,
...
pub async fn delete(
    ...
    sqlx::query!(
        "DELETE FROM event_modifiers WHERE id = $1",
        modifier_id
    )
    .execute(&data.pool)
    .await?;
